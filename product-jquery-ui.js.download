...
$.fn.extend({
	disableSelection: function() {
		return this.bind( ( $.support.selectstart ? "selectstart" : "mousedown" ) +
			".ui-disableSelection", function( event ) {event.preventDefault();});
	},
					...
					this._delay(function() {
						...
						this.document.one( "mousedown", function( event ) {
							if ( event.target !== that.element[ 0 ] &&
									...
									!$.contains( menuElement, event.target ) ) {that.close();}
						});
					});
				}
			},
...
$.widget( "ui.button", {
	...
	_create: function() {
		...
		if ( toggleButton ) {
			this.element.bind( "change" + this.eventNamespace, function() {that.refresh();});
		}
...
$.extend(Datepicker.prototype, {
	...
	_optionDatepicker: function(target, name, value) {
		...
		if (inst) {
			...
			if ( "disabled" in settings ) {
				if ( settings.disabled ) {this._disableDatepicker(target);} else {this._enableDatepicker(target);}
			}
	...
	_checkExternalClick: function(event) {
		...
		if ( ( ( $target[0].id !== $.datepicker._mainDivId &&
			...
			( $target.hasClass($.datepicker.markerClassName) && $.datepicker._curInst !== inst ) ) {$.datepicker._hideDatepicker();}
	},
			...
			if (literal) {
				...
				switch (format.charAt(iFormat)) {
					...
					case "!":
						...
						break;
					case "'":
						if (lookAhead("'")){checkLiteral();} else {
							literal = true;
						}
						break;
					default:
...
$.widget("ui.draggable", $.ui.mouse, {
	...
	_mouseStop: function(event) {
		...
		if((this.options.revert === "invalid" && !dropped) || (this.options.revert === "valid" && dropped) || this.options.revert === true || ($.isFunction(this.options.revert) && this.options.revert.call(this.element, dropped))) {
			...
			if(this._trigger("stop", event) !== false) {this._clear();}
		}
...
$.widget( "ui.menu", {
	...
	_create: function() {
		...
		this._on({
			...
			"mousedown .ui-menu-item > a": function( event ) {event.preventDefault();},
	...
	focus: function( event, item ) {
		...
		if ( event && event.type === "keydown" ) {this._close();} else {
...
$.widget( "ui.slider", $.ui.mouse, {
	...
	_refreshValue: function() {
		...
		if ( this.options.values && this.options.values.length ) {
			...
			$(".range .inner .mask").css("right", (100-valPercent)+"%");
